...
class ShapeOPTDecoder(OPTDecoder):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None: # when  train and first generate
            ...
            face_embeds = self.token_embed_positions(attention_mask=None, face_ids=face_ids, input_ids=input_ids,
                                                     face_per_token = self.face_per_token) # when generation input_ids is bos here
            embeds_from_id += face_embeds
    ...
    def forward(self, attention_mask=None, face_ids = None, input_ids = None, face_per_token = None):
        ...
        for cur_batch_index in range(batch_size):
            ...
            if idx_in_extra:
                ...
                if cur_ids == self.n_discrete_size + 3:
